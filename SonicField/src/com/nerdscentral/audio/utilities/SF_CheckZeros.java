...
public class SF_CheckZeros implements SFPL_Operator, SFPL_RefPassThrough
{
    ...
    public Object Interpret(Object input) throws SFPL_RuntimeException
    {
        ...
        for (int i = 0; i < len; ++i)
        {
            if (in.getSample(i) == 0)
            {
                System.out.println(Messages.getString("SF_CheckZeros.1") + i + Messages.getString("SF_CheckZeros.2") + (len - 1) //$NON-NLS-1$ //$NON-NLS-2$
                                + Messages.getString("SF_CheckZeros.3"));  //$NON-NLS-1$
            }
        }
        return in;
    }
